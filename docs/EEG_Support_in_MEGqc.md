...
| `meg_qc/plotting/meg_qc_dataset_plots.py` | Dataset-level QA report |
| `meg_qc/plotting/meg_qc_dataset_qc_plots.py` | Dataset-level QC report |
...
### Dataset Reports (QA and QC)

Generated by `meg_qc_dataset_plots.py` and `meg_qc_dataset_qc_plots.py`. Tab order:

```python
tab_order = ["Combined (mag+grad)", "MAG", "GRAD", "EEG"]
```

Only tabs with actual data are shown. The dataset reports can split MEG and EEG data into separate report files:
...
Modality detection in dataset reports uses path-based inference:
...
│  Dataset QA:     EEG tab in dataset QA report                    │
│  Dataset QC:     Separate EEG report file if EEG data exists   │
└──────────────────────────────────────────────────────────────┘
```
